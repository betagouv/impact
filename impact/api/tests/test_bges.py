...
from api.bges import last_reporting
...
def test_une_seule_annee_de_publication_trouvee(mocker):
    ...
    last_reporting_data = last_reporting(SIREN)

    assert last_reporting_data["year"] == 2021
    assert last_reporting_data["publication_date"] == date(2022, 5, 12)
...
def test_aucune_annee_de_publication(mocker):
    ...
    assert last_reporting_data == None
    faked_request.assert_called_once_with(
        "https://bilans-ges.ademe.fr/api/inventories",
        params={"page": "1", "itemsPerPage": "11", "entity.siren": SIREN},
    )
...
def test_deux_annees_de_publication_trouvees(mocker):
    ...
    assert last_reporting_data["year"] == 2022
    assert last_reporting_data["publication_date"] == date(2023, 12, 14)
    faked_request.assert_called_once_with(
        "https://bilans-ges.ademe.fr/api/inventories",
        params={"page": "1", "itemsPerPage": "11", "entity.siren": SIREN},
    )
...
def test_echec_erreur_de_l_api(code_http, mocker):
    ...
    assert last_reporting_data is None
