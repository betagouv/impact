...
class Reglementation(ABC):
    ...
    def calculate_status_for_anonymous_user(
        ...
        if cls.est_soumis(caracteristiques):
            ...
            status_detail = f"""Vous êtes soumis à cette réglementation si {', '.join(cls.criteres_remplis(caracteristiques))}. <a href="{login_url}">Connectez-vous pour en savoir plus.</a>"""
        else:
